// paste-shots GNOME Shell extension (GNOME 42–44 / imports API).
//
// Provides two things:
//   1. A session-bus DBus service `org.pasteshots.Shell` that lets the
//      paste-shots tool snapshot / raise windows from user-space — which is
//      otherwise impossible on GNOME Wayland.
//   2. An optional floating icon widget on the Shell chrome layer that is
//      always above other windows, draggable, and persists its position.
//
// Lifecycle: enable() wires the DBus + focus listener, disable() tears
// everything down. No state survives disable.

const { St, Gio, GLib, Meta, Shell, Clutter } = imports.gi;
const Main = imports.ui.main;
const PopupMenu = imports.ui.popupMenu;
const ExtensionUtils = imports.misc.extensionUtils;

// We register an object on GNOME Shell's existing session-bus name
// (org.gnome.Shell) rather than owning our own bus name — that's the
// standard pattern for shell extensions (same one used by window-calls,
// gTile, etc.) and sidesteps several known gjs bus_own_name quirks.
...
// Matched against WM_CLASS (lowercased) to decide what counts as a paste
// target. IDE classes (code, cursor) are deliberately excluded — they
// expose a single window with internal panes (editor / terminal / etc),
// and paste-shots can only address the whole window. Pinning an IDE
// makes Ctrl+V land in whichever pane was last focused, which is
// usually wrong. Run Claude Code in a real terminal app and pin that.
...
function _activate(w) {
    ...
    try {
        ...
        log(`[paste-shots] activate failed: ${e}`);
        return false;
    }
}


class PasteShotsExtension {
    enable() {
        log('[paste-shots] enable() starting');
        ...
        log('[paste-shots] enable() complete — DBus exported on ' + OBJECT_PATH);
    }
    ...
    _connectFocusTracker() {
        ...
        // Seed with whatever is focused right now, if it's a terminal.
        const w = global.display.focus_window;
        if (_isTerminal(w)) this._lastTerminalWin = w;
    }
    ...
    SnapshotFocused() {
        ...
        // Refuse non-terminals so the pin target can't accidentally be
        // set to e.g. a code editor whose internal pane focus we can't
        // control. Empty string → tray shows "not a terminal" notification.
    ...
    RaiseLastTerminal() {
        if (this._lastTerminalWin) {
            try {
                // Validate the cached ref is still alive.
                this._lastTerminalWin.get_id();
            } catch (_) {
                this._lastTerminalWin = null;
            }
        }
        ...
        // Fallback: scan current windows for any terminal and activate the most recent.
        const actors = global.get_window_actors();
        const candidates = [];
        for (let i = 0; i < actors.length; i++) {
            const w = actors[i].meta_window;
    ...
    _persistWidgetState(patch) {
        ...
        try {
            ...
            log(`[paste-shots] persist state failed: ${e}`);
        }
    }
    ...
    _showWidget() {
        ...
        this._widget = new FloatingWidget({
            onClick: () => {
                GLib.spawn_command_line_async(CLI_PATH);
            },
    ...
    _buildMenu() {
        ...
        // Right-click context menu — only entry point to settings/quit when
        // tray_icon=False, since the AppIndicator menu is not available.
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            m.connect('activate', () => {
                ...
                catch (e) { log(`[paste-shots] cmd failed: ${cmd} — ${e}`); }
            });
...
// Draggable floating button, rendered on the Shell chrome layer.
// We extend St.BoxLayout (not St.Widget) so the container auto-sizes to its
// children; a bare St.Widget with no layout manager lays out at 0x0.
const FloatingWidget = imports.gi.GObject.registerClass({
    ...
    _onPress(_actor, event) {
        if (event.get_button() === 3) {
            // Right-click → context menu. Critical when tray_icon=False;
            // otherwise the AppIndicator menu is the only way to reach
            // Settings/Quit.
...
function init() {
    return new PasteShotsExtension();
}
